...
import { 
  ...
  Building, TrendingDown, Copy, Heart
} from 'lucide-react';
...
const SectionHeader = ({ title, icon: Icon, description }) => {
  const handleCopy = () => {
    ...
    const text = `我正在使用 FinKit 計算【${title}】，推薦你也試試！\n${url}`;
    navigator.clipboard.writeText(text).then(() => alert("已複製推薦文案！"));
  };
    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
            ...
            <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 max-w-lg">
                ...
                <InputGroup label="聯絡電話" value={phone} onChange={setPhone} placeholder="0912-345-678" />
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
            ...
            <div className="grid md:grid-cols-2 gap-6">
                <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700">
                    ...
                    <InputGroup label="履約價 (Strike)" value={strikePrice} onChange={setStrikePrice} prefix="$" />
                ...
                <div className="space-y-4">
                    ...
                    <ResultCard title="損益兩平股價" value={`$${breakEvenPrice.toFixed(2)}`} subtext={`下跌 ${(100 - (breakEvenPrice/strikePrice)*100).toFixed(1)}% 內不賠錢`} highlight={true} />
                </div>
            </div>
        </div>
    );
