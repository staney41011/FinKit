                ...
                <div className="lg:col-span-7 space-y-6">
                                ...
                                <div key={asset.id} className="flex gap-2 items-center bg-slate-50 p-2 rounded-xl border border-slate-100">
                                    ...
                                    <div className="flex-1 grid grid-cols-2 gap-2 max-w-[320px]">
                                        <div className="h-10 bg-white border border-slate-300 rounded-lg flex items-center px-2 focus-within:ring-1 focus-within:ring-amber-500">
                                            <input 
                                                type="text" placeholder="代號" 
                                                value={asset.code} onChange={(e)=>updateAsset(asset.id, 'code', e.target.value)} 
                                        ...
                                        <div className="h-10 bg-white border border-slate-300 rounded-lg flex items-center px-2 focus-within:ring-1 focus-within:ring-amber-500 relative">
                                            <input 
                                                ...
                                                value={asset.price} onChange={(e)=>updateAsset(asset.id, 'price', e.target.value)} 
                                    ...
                                    <button onClick={()=>removeAsset(asset.id)} className="w-8 h-8 flex items-center justify-center text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors">
                                        <Trash2 size={16}/>
                                    </button>
                                </div>
                            ))}
                    ...
                    <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200">
                        ...
                        <div className="grid grid-cols-3 gap-2">
                            ...
                            <InputGroup label="KI" value={kiPct} onChange={setKiPct} suffix="%" />
                ...
                <div className="lg:col-span-5 space-y-4">
                    <div className="p-5 bg-slate-800 text-white rounded-xl shadow-lg border border-slate-700 relative overflow-hidden">
                        ...
                        <div className="flex justify-between items-end mb-3 border-b border-slate-600 pb-3">
                            ...
                            <div className="text-right"><p className="text-xs text-slate-400">總配息</p><p className="text-xl font-mono text-amber-400">${fmt(totalCoupon)}</p></div>
                        </div>
                        <div className="grid grid-cols-2 gap-4 text-[10px] text-slate-300">
                             <div><span className="block text-slate-500">損益兩平</span> Strike × {(1 - (yieldRate/100)*(months/12)).toFixed(2)}</div>
                             <div className="text-right"><span className="block text-slate-500">最大損失</span> 本金歸零</div>
                        </div>
                    </div>
                    <div className="space-y-3">
                        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider ml-1">價位檢核</p>
                        {assets.map((asset) => {
...
const RiskPremiumCalculator = () => {
    const [nar, setNar] = useStickyState(1000000, 'v6_risk_nar');
    const [age, setAge] = useStickyState(30, 'v6_risk_age');
    const [gender, setGender] = useStickyState('male', 'v6_risk_gender');
    const [table, setTable] = useStickyState('6', 'v6_risk_table');

    const baseMortality = { male: [46, 28, 22, 35, 60, 95, 145, 215, 320, 480, 750, 1150, 1800, 2800], female: [40, 18, 12, 18, 30, 50, 75, 110, 160, 250, 400, 650, 1050, 1700] };
    const getMortalityRate = (a, g, t) => {
        ...
        return (ratePer100k / 10) * (multipliers[t] || 1);
    };

    const rate = getMortalityRate(Number(age), gender, table);
    const monthlyPremium = Math.round((Number(nar) / 10000) * rate);
    const annualPremium = monthlyPremium * 12;

    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
            <SectionHeader title="危險保費 (COI)" icon={HeartPulse} description="採用台灣經驗生命表，計算自然費率。" />
            <div className="grid md:grid-cols-2 gap-8">
                <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 h-fit">
                    <InputGroup label="危險保額 (NAR)" value={nar} onChange={setNar} prefix="$" />
                    
                    <div className="flex gap-4 mb-5 max-w-[240px]">
                        <div className="flex-1">
                            <label className="block text-xs uppercase tracking-wider font-bold text-slate-500 mb-2 ml-1">年齡</label>
                            <input type="number" value={age} onChange={(e)=>setAge(e.target.value)} className="w-full p-2.5 border border-slate-300 rounded-xl font-mono text-sm focus:ring-amber-500" />
                        </div>
                        <div className="flex-1">
                            <label className="block text-xs uppercase tracking-wider font-bold text-slate-500 mb-2 ml-1">性別</label>
                            <select value={gender} onChange={(e)=>setGender(e.target.value)} className="w-full p-2.5 border border-slate-300 rounded-xl bg-white text-sm focus:ring-amber-500">
                    ...
                    <div className="mb-5 max-w-[240px]">
                        <label className="block text-xs uppercase tracking-wider font-bold text-slate-500 mb-2 ml-1">生命表</label>
                        <select value={table} onChange={(e)=>setTable(e.target.value)} className="w-full p-2.5 border border-slate-300 rounded-xl bg-white text-sm focus:ring-amber-500 font-mono">
                ...
                <div className="space-y-4">
                    <ResultCard title="每月危險保費" value={`$${fmt(monthlyPremium)}`} highlight={true} colorClass="text-red-500" />
                    <ResultCard title="預估年繳總額" value={`$${fmt(annualPremium)}`} />
                    <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-2 font-mono text-[10px] text-slate-600">
                        <div className="flex justify-between"><span>費率 (每萬保額)</span><span>${rate.toFixed(4)}</span></div>
                        <div className="flex justify-between"><span>計價單位</span><span>{fmt(nar/10000)}</span></div>
                    </div>
                    <div className="p-2 bg-blue-50 text-blue-700 text-[10px] rounded-lg text-center font-mono">公式：(保額/10000) × 費率</div>
                </div>
            </div>
        </div>
    );
...
// ... (Compound, Dividend, DCA, Stock, Forex, RentVsBuy, Fire, Insurance, Inflation 模組全部縮小 InputGroup) ...
