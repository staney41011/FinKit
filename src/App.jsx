...
import { 
  ...
  Save, ArrowRightLeft, Download, HeartPulse
} from 'lucide-react';
...
const ResultCard = ({ title, value, subtext, highlight = false, colorClass = "text-amber-600" }) => (
  <div className={`p-5 rounded-xl border transition-all duration-300 relative overflow-hidden group h-full flex flex-col justify-center min-h-[120px]
    ...
    {subtext && <p className="text-xs text-slate-400 mt-2 border-t border-slate-100 pt-2 leading-relaxed">{subtext}</p>}
  </div>
);
  ...
  return (
    <div className="w-full mt-6 mb-4 select-none overflow-hidden pr-2 pl-10">
      ...
      >
        ...
        {hoverVal && (
            <div className="absolute bg-slate-800/95 backdrop-blur text-white text-xs p-3 rounded-lg shadow-xl z-20 pointer-events-none whitespace-nowrap border border-slate-600"
                ...
                <div className="space-y-0.5">
                    <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full" style={{background:color}}></div><span className="font-mono">${fmt(hoverVal.val1)}</span></div>
                    {hoverVal.val2 && (<div className="flex items-center gap-2 text-slate-400"><div className="w-2 h-2 rounded-full bg-slate-400"></div><span className="font-mono">${fmt(hoverVal.val2)}</span></div>)}
                </div>
            </div>
        )}
      </div>
      <div className="flex justify-between text-xs text-slate-400 mt-2 font-mono px-1"><span>Start</span><span>End ({data.length-1})</span></div>
    </div>
  );
};

// ==========================================
// 1. 危險保費計算機 (New Feature)
// ==========================================
const RiskPremiumCalculator = () => {
    const [nar, setNar] = useStickyState(1000000, 'v6_risk_nar'); // 危險保額
    const [age, setAge] = useStickyState(30, 'v6_risk_age');
    const [gender, setGender] = useStickyState('male', 'v6_risk_gender');
    const [table, setTable] = useStickyState('6', 'v6_risk_table'); // 第6回

    // 簡化的第6回生命表 (2021) 每10萬人死亡率 (q_x * 100000)
    // 數據為模擬趨勢，非精確精算表，僅供趨勢參考
    const baseMortality = {
        male: [46, 28, 22, 35, 60, 95, 145, 215, 320, 480, 750, 1150, 1800, 2800], // 0, 10, 20...
        female: [40, 18, 12, 18, 30, 50, 75, 110, 160, 250, 400, 650, 1050, 1700]
    };

    // 插值計算死亡率 (Per 10,000)
    const getMortalityRate = (a, g, t) => {
        const data = g === 'male' ? baseMortality.male : baseMortality.female;
        const idx = Math.min(Math.floor(a / 10), data.length - 2);
        const ratio = (a % 10) / 10;
        const ratePer100k = data[idx] + (data[idx+1] - data[idx]) * ratio;
        
        // 歷史生命表加權 (越舊越貴)
        const multipliers = { '6': 1.0, '5': 1.25, '4': 1.55, '3': 2.1 };
        return (ratePer100k / 10) * (multipliers[t] || 1); // 轉為每萬人費率
    };

    const rate = getMortalityRate(Number(age), gender, table);
    const monthlyPremium = Math.round((Number(nar) / 10000) * rate); // 危險保費公式
    const annualPremium = monthlyPremium * 12;

    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
            <SectionHeader title="投資型保單危險保費試算" icon={HeartPulse} description="採用台灣經驗生命表，計算自然費率下的危險成本(COI)。" />
            
            <div className="grid md:grid-cols-2 gap-8">
                <div className="bg-white p-6 rounded-2xl shadow-lg border border-slate-100 h-fit">
                    <InputGroup label="危險保額 (NAR)" value={nar} onChange={setNar} prefix="$" placeholder="淨危險保額" />
                    
                    <div className="flex gap-4 mb-5 max-w-[280px]">
                        <div className="flex-1">
                            <label className="block text-xs uppercase tracking-wider font-bold text-slate-500 mb-2 ml-1">年齡</label>
                            <input type="number" value={age} onChange={(e)=>setAge(e.target.value)} className="w-full p-3 border border-slate-300 rounded-xl font-mono text-sm focus:ring-amber-500" />
                        </div>
                        <div className="flex-1">
                            <label className="block text-xs uppercase tracking-wider font-bold text-slate-500 mb-2 ml-1">性別</label>
                            <select value={gender} onChange={(e)=>setGender(e.target.value)} className="w-full p-3 border border-slate-300 rounded-xl bg-white text-sm focus:ring-amber-500">
                    ...
                    <div className="mb-5 max-w-[280px]">
                        <label className="block text-xs uppercase tracking-wider font-bold text-slate-500 mb-2 ml-1">使用生命表</label>
                        <select value={table} onChange={(e)=>setTable(e.target.value)} className="w-full p-3 border border-slate-300 rounded-xl bg-white text-sm focus:ring-amber-500 font-mono">
                            <option value="6">第六回 (2021) - 最新</option>
                            <option value="5">第五回 (2012)</option>
                            <option value="4">第四回 (2004)</option>
                            <option value="3">第三回 (1989)</option>
                        </select>
                    </div>
                </div>

                <div className="space-y-4">
                    <ResultCard title="每月危險保費 (COI)" value={`$${fmt(monthlyPremium)}`} highlight={true} colorClass="text-red-500" />
                    <ResultCard title="預估年繳總額" value={`$${fmt(annualPremium)}`} />
                    
                    <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-2 font-mono text-xs text-slate-600">
                        <div className="border-b border-slate-200 pb-1 mb-2 font-bold text-slate-400 uppercase text-[10px]">計算明細</div>
                        <div className="flex justify-between"><span>參考費率 (每萬保額)</span><span>${rate.toFixed(4)}</span></div>
                        <div className="flex justify-between"><span>計價單位數</span><span>{fmt(nar/10000)} 單位</span></div>
                    </div>
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
      ...
      <div className="flex justify-end no-print mb-2">
         <button onClick={()=>setCompareMode(!compareMode)} className={`text-xs px-3 py-1.5 rounded-full border transition-all ${compareMode ? 'bg-amber-600 text-white border-amber-600' : 'text-slate-500 border-slate-300 hover:border-amber-500 hover:text-amber-600'}`}>
...
const StockCalculator = () => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
      ...
      <div className="grid md:grid-cols-2 gap-8">
        <div className="bg-white p-6 rounded-2xl shadow-lg border border-slate-100 h-fit">
          <div className="mb-6 max-w-[280px]"><label className="block text-xs uppercase tracking-wider font-bold text-slate-500 mb-2 ml-1">交易種類</label><div className="grid grid-cols-2 gap-2">{[{id:'stock', name:'個股 (0.3%)'}, {id:'day', name:'當沖 (0.15%)'}, {id:'etf', name:'ETF (0.1%)'}, {id:'bond', name:'債券ETF (0%)'}].map(t => (<button key={t.id} onClick={()=>setType(t.id)} className={`py-2 text-[10px] font-bold rounded-lg border transition-all ${type===t.id ? 'bg-amber-600 text-white border-amber-600 shadow-md' : 'text-slate-500 border-slate-200 bg-slate-50 hover:bg-white'}`}>{t.name}</button>))}</div></div>
        ...
        <div className="space-y-4">
          ...
          <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-2 font-mono text-xs text-slate-600">
              <div className="border-b border-slate-200 pb-1 mb-2 font-bold text-slate-400 uppercase tracking-widest text-[10px]">交易明細 (Breakdown)</div>
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
            ...
            <div className="grid md:grid-cols-2 gap-8">
                <div className="bg-white p-6 rounded-2xl shadow-lg border border-slate-100 h-fit">
                    ...
                    <div className="mb-4 max-w-[280px]"><div className="flex justify-between mb-1 ml-1"><label className="text-xs font-bold text-slate-500">成交匯率</label><span className="text-[10px] text-blue-500 cursor-pointer hover:underline" onClick={()=>setManualRate('')}>重置為即時匯率</span></div><input type="number" step="0.0001" value={manualRate || (sysRate ? sysRate.toFixed(4) : '')} onChange={(e) => setManualRate(e.target.value)} className="w-full p-3 border border-slate-300 rounded-lg font-mono text-sm focus:ring-2 focus:ring-amber-500 outline-none"/><p className="text-[10px] text-slate-400 mt-1.5 flex items-center gap-1 ml-1">{loading ? <RefreshCw className="animate-spin" size={10}/> : null} {sysRate ? `1 ${fromCurr} ≈ ${sysRate.toFixed(4)} ${toCurr}` : '載入中...'}</p></div>
                </div>
...
const FinancialToolkit = () => {
  ...
  const menuCategories = [
    ...
    { title: "實用工具", items: [{ id: 'risk', name: '危險保費', icon: HeartPulse }, { id: 'forex', name: '匯率換算', icon: Globe }, { id: 'fcn', name: 'FCN 試算', icon: PieChart }, { id: 'dca', name: '定期定額', icon: RefreshCw }, { id: 'irr', name: 'IRR', icon: Percent }, { id: 'inf', name: '通膨試算', icon: TrendingDown }] },
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'risk': return <RiskPremiumCalculator />;
