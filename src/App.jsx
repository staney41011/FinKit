        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
            ...
            <div className="grid md:grid-cols-2 gap-8">
                <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 h-fit">
                    ...
                    <div className="mb-4 max-w-[220px]"><label className="block text-xs uppercase tracking-wider font-bold text-slate-500 mb-1.5 ml-1">配息頻率</label><div className="flex gap-2">{[{v:1, l:'年配'}, {v:2, l:'半年'}, {v:4, l:'季配'}, {v:12, l:'月配'}].map(o => (<button key={o.v} onClick={()=>setFreq(o.v)} className={`flex-1 py-2 text-xs rounded-lg border transition-colors ${freq===o.v ? 'bg-amber-600 text-white border-amber-600' : 'bg-slate-50 text-slate-500 border-slate-200 hover:bg-white'}`}>{o.l}</button>))}</div></div>
                </div>
                <div className="space-y-4">
...
const StockCalculator = () => {
  ...
  const calculate = () => {
    ...
    const buySettlement = buyVal + buyFee; const sellSettlement = sellVal - sellFee - tax;
...
const ForexCalculator = () => {
    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
            ...
            <div className="grid md:grid-cols-2 gap-8">
                <div className="bg-white p-5 rounded-2xl shadow-lg border border-slate-100 h-fit">
...
const ProfileSettings = ({ settings, onUpdate }) => {
    const [name, setName] = useState(settings.name); const [line, setLine] = useState(settings.line); const [phone, setPhone] = useState(settings.phone);
