...
import { 
  ...
  Save, ArrowRightLeft, Download, Layers, Check
} from 'lucide-react';
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
            ...
            <div className="grid md:grid-cols-2 gap-8">
                <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 h-fit">
                    ...
                    {deductHealth && <p className="text-[10px] text-slate-400 ml-9 mt-0.5">註：單次配息達 2 萬元始扣取</p>}
                </div>
                ...
                <div className="space-y-4">
                    ...
                    <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-2 font-mono text-[10px] text-slate-600">
                         <div className="flex justify-between"><span>投入本金</span><span>${fmt(principal)}</span></div>
                         <div className="flex justify-between"><span>單位價格</span><span>${fmt(price)}</span></div>
                         <div className="border-b border-slate-200 my-1"></div>
                         <div className="flex justify-between"><span>原始配息總額</span><span>${fmt(grossIncome)}</span></div>
                         <div className="flex justify-between text-red-500"><span>二代健保補充保費</span><span>-${fmt(healthFee)}</span></div>
                         <div className="flex justify-between font-bold text-slate-800 pt-1 border-t border-slate-200 mt-1"><span>實領配息金額</span><span>${fmt(netIncome)}</span></div>
                    </div>
                </div>
            </div>
        </div>
