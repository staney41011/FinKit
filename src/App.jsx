...
const ResultCard = ({ title, value, subtext, highlight = false, colorClass = "text-amber-600" }) => (
  <div className={`p-5 rounded-xl border transition-all duration-300 relative overflow-hidden group h-full flex flex-col justify-center
...
const SectionHeader = ({ title, icon: Icon, description }) => {
  ...
  const handleCapture = async () => {
    ...
    // 1. 建立一個虛擬沙盒 (Sandbox)
    // 這是一個看不到的容器，固定寬度 1080px，用來模擬電腦版排版
    const sandbox = document.createElement("div");
    sandbox.style.position = "fixed";
    sandbox.style.top = "0";
    sandbox.style.left = "-9999px"; // 藏在螢幕外
    sandbox.style.width = "1080px"; // 強制寬度，解決手機右邊空白問題
    ...
    // 將克隆體放入沙盒，再將沙盒放入 body
    sandbox.appendChild(clone);
    document.body.appendChild(sandbox);
    
    // 4. 等待圖片與字型載入 (給瀏覽器一點時間重繪)
    ...
    <div className="mb-8 flex justify-between items-start border-b border-slate-200 pb-6 no-print">
      ...
      <div className="flex gap-2">
        <button 
            ...
            className="px-4 py-2 bg-slate-800 text-white text-sm font-medium rounded-lg hover:bg-slate-700 transition-colors flex items-center gap-2 shadow-md disabled:opacity-50 whitespace-nowrap"
        >
...
const InteractiveChart = ({ data, color="#d97706", data2, title="資產走勢" }) => {
  ...
  return (
    <div className="w-full mt-8 mb-4 select-none pl-10 pr-2 relative z-0">
      <div 
        ref={containerRef}
...
const StockCalculator = () => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
          <div className="mb-6"><label className="block text-xs uppercase tracking-wider font-bold text-slate-500 mb-3 ml-1">交易種類</label><div className="grid grid-cols-2 gap-3">{[{id:'stock', name:'個股 (0.3%)'}, {id:'day', name:'當沖 (0.15%)'}, {id:'etf', name:'ETF (0.1%)'}, {id:'bond', name:'債券ETF (0%)'}].map(t => (<button key={t.id} onClick={()=>setType(t.id)} className={`py-2.5 text-xs font-medium rounded-lg border transition-all ${type===t.id ? 'bg-amber-600 text-white border-amber-600 shadow-md' : 'text-slate-500 border-slate-200 bg-slate-50 hover:bg-white'}`}>{t.name}</button>))}</div></div>
          ...
          <div className="mb-5 group">
             <label className="block text-xs uppercase tracking-wider font-bold text-slate-500 mb-3 ml-1">手續費折數 ({(discount/10).toFixed(1)}折)</label>
        ...
        <div className="space-y-4">
          ...
          <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-3">
              <div className="flex justify-between items-center text-sm">
                  <span className="text-slate-500 font-bold">買進交割 (支出)</span>
                  <span className="font-mono text-slate-700">${fmt(res.buySettlement)}</span>
              </div>
              <div className="flex justify-between items-center text-sm border-t border-slate-200 pt-2">
                  <span className="text-slate-500 font-bold">賣出交割 (入帳)</span>
                  <span className="font-mono text-slate-700">${fmt(res.sellSettlement)}</span>
              </div>
          </div>

          <div className="grid grid-cols-2 gap-4">
              <div className="p-3 bg-white border border-slate-200 rounded-lg text-center">
                  <p className="text-[10px] text-slate-400 font-bold">總手續費</p>
                  <p className="font-mono text-slate-600">${res.buyFee + res.sellFee}</p>
              </div>
              <div className="p-3 bg-white border border-slate-200 rounded-lg text-center">
                  <p className="text-[10px] text-slate-400 font-bold">證交稅</p>
                  <p className="font-mono text-slate-600">${res.tax}</p>
              </div>
          </div>
        </div>
      </div>
            ...
            <div className="grid md:grid-cols-2 gap-6">
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                    <div className="flex items-center gap-2 mb-4">
                        <div className="flex-1"><label className="block text-xs font-bold text-slate-500 mb-2">持有 (From)</label><select value={fromCurr} onChange={(e)=>setFromCurr(e.target.value)} className="w-full p-3 border border-slate-300 rounded-xl bg-slate-50 text-sm font-bold focus:ring-amber-500">{currencies.map(c=><option key={c.c} value={c.c}>{c.c} {c.n}</option>)}</select></div>
...
const ProfileSettings = () => {
    ...
    const handleSave = () => {
        ...
        // 強制寫入並延遲刷新
        setTimeout(() => {
            window.localStorage.setItem('v4_name', JSON.stringify(name));
            window.localStorage.setItem('v4_line', JSON.stringify(line));
            window.localStorage.setItem('v4_phone', JSON.stringify(phone));
            ...
            <div className="bg-white p-6 rounded-2xl border border-slate-200 max-w-lg">
                ...
                <div className="mt-8 pt-4 border-t border-slate-100">
                    <button 
                        onClick={handleSave} 
                        disabled={saving}
                        className="w-full py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl shadow-lg shadow-blue-200 flex items-center justify-center gap-2 transition-all active:scale-95 disabled:opacity-50"
                    >
                        {saving ? <RefreshCw className="animate-spin" size={18}/> : <Save size={18} />} 
                        {saving ? '儲存中...' : '儲存設定並重新整理'}
                    </button>
...
// ... (以下為 FCN 和其他 Calculator，維持 v5.2 的邏輯但已整合進來) ...
...
const FinancialToolkit = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800 flex flex-col md:flex-row print:bg-white print:text-black selection:bg-amber-100 overflow-x-hidden">
      ...
      <main className="flex-1 p-4 md:p-10 overflow-y-auto print:p-0 print:overflow-visible h-screen bg-slate-50 overflow-x-hidden">
